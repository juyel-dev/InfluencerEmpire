...
| 2026-07-10 | UI Redesign — Map & Design System | Applied ui-ux-pro-max "Modern Dark (Cinema Mobile)" system. Tokens: playful display font **Fredoka** + body **Nunito** (was Inter); signature **rose→violet→cyan** brand gradient + `.brand-gradient`/`.text-gradient`/`.glass-panel`/`.glow-brand` utilities. New `AuroraBackground` (drifting brand-color ambient blobs) mounted in `GameShell`. `MapScreen` fully redesigned: gradient-ring avatar, gradient follower count with shimmer progress-to-10K, glass stat tiles with Lucide icons (Zap/DollarSign/Sparkles/Heart), NPC glass chips via SVG `CharacterPortrait` (Lucide `UserRound` fallback), bento location grid with Lucide icons replacing emojis, gradient End-Day CTA. **Removed emoji as structural icons** per skill rules; only content mascots remain (none on chrome). Respects `prefers-reduced-motion`. |
